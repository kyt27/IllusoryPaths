    ...
    private void ExpandFrontier(Node node) {
        ...
        for (int i = 0; i < node.Edges.Count; i++) {
            ...
            if (node.Edges[i] == null ||
                node.Edges.Count == 0 ||
                exploredNodes.Contains(node.Edges[i].neighbour) ||
                frontierNodes.Contains(node.Edges[i].neighbour)) {
                continue;
            }

            // create PreviousNode breadcrumb trail if Edge is active
            if (node.Edges[i].isActive && node.Edges[i].neighbour != null) {
                node.Edges[i].neighbour.PreviousNode = node;

                // add neighbor Nodes to frontier Nodes
                frontierNodes.Add(node.Edges[i].neighbour);
            }
        }
    }
